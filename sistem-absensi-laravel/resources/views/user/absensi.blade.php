            ...
            <form id="formAbsensi" onsubmit="submitAbsensi(event)">
              ...
              @if($action === 'pulang')
                <div class="mb-4">
                  ...
                  <textarea name="kendala_hari_ini" id="kendala" rows="2" class="form-control"
                    placeholder="Tulis kendala pekerjaan hari ini..."></textarea>
                </div>
                    ...
                    <div class="card-body p-3" style="max-height: 200px; overflow-y: auto;">
                      @forelse($tugas ?? [] as $t)
                        <div class="form-check mb-2 border-bottom pb-2">
                          <input class="form-check-input" type="checkbox" name="tugas_ids[]" value="{{ $t->id }}"
                            id="tugas_{{ $t->id }}">
                          <label class="form-check-label d-block" for="tugas_{{ $t->id }}">
                            ...
                            @if($t->kategori)
                              <span class="badge bg-light text-secondary border ms-1"
                                style="font-size:0.65rem;">{{ $t->kategori }}</span>
                            @endif
                          </label>
                        </div>
                      @empty
                ...
                <div class="mb-4 text-start">
                  ...
                  <textarea name="todo_note" class="form-control" rows="2"
                    placeholder="Catatan tambahan mengenai pekerjaan hari ini..."></textarea>
                </div>
              @endif

              <button type="submit" class="btn btn-primary w-100 py-2 rounded-pill fw-bold shadow-sm" id="btnSubmitForm"
                disabled>
                <span id="btnText"><i
                    class="bi {{ $action === 'masuk' ? 'bi-box-arrow-in-right' : 'bi-box-arrow-right' }} me-2"></i>Kirim
                  Absensi</span>
                <span id="btnSpinner" class="d-none"><span
...
@section('scripts')
  @if($action !== 'done')
    <script>
      ...
      // Ganti bagian submit via AJAX di script Anda
        ...
        try {
          const response = await fetch("{{ route('user.absensi.store') }}", {
            ...
            headers: {
              ...
              "X-CSRF-TOKEN": "{{ csrf_token() }}" // Pastikan token terkirim eksplisit
            },
            body: formData
          });

          // Cek jika response bukan JSON (sering terjadi jika ada error PHP/HTML yang bocor)
          ...
          try {
            ...
            alert("Terjadi kesalahan sistem di server. Cek console log.");
            resetBtn();
            return;
          }
          ...
          if (data.ok) {
            ...
            // Gunakan fallback jika msg tidak ada
            alert("Gagal: " + (data.msg || "Pesan tidak diketahui"));
            resetBtn();
          }
        } catch (error) {
